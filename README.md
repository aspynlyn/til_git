...
- `til_git 폴더는 clone을 할 필요가 있나?`

  - til_git은 이미 git 세팅이 되어있음 clone 필요없음

#### 5-5. 기존 프로젝트에서 GitHub 브랜치 적용

- 기존 프로젝트에서는 clone 하지 않음

- 기존 프로젝트에서는 `fetch`사용

- 1. fetch로 github의 모든 브랜치 가져옴

```bash
git fetch --all
```

- 2. 브랜치 목록 확인(로컬과 깃허브 브랜치 모두)
